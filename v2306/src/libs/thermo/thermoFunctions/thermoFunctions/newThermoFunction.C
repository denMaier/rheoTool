...
{
    const dictionary sDict(dict.subOrEmptyDict(name));

    word typeName(sDict.getOrDefault<word>("type","Constant"));
    ...
    return autoPtr<thermoFunction>(ctorPtr(name, mesh, sDict));
}
